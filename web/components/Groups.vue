...
import { type Group, type GetGroupsQuery, GetGroupsSort } from "~/models/group";
...
const query: GetGroupsQuery = {
  ...
  desc: true,
  sort: GetGroupsSort.Title,
  extended: true,
};
updateQueryFromValues();
let { data: groups, refresh } = await getGroups(query);
let page = ref(1);
const maxPage = Math.round((groups.value?.data.total ?? 0) / query.limit);
...
async function clickGroup(e: Event, group: Group) {
  e.preventDefault();
...
<template>
  <n-space vertical>
    <n-pagination
      simple
      v-if="!hide_pagination && groups?.data.total"
      v-model:page="page"
      :page-size="limit ?? config.public.limitData.any"
      :item-count="max ?? groups.data.total"
    ...
    <n-list v-if="groups">
      <n-list-item v-if="groups.data.items.length == 0">
        <n-space vertical>
          <span>No groups yet~</span>
        </n-space>
      </n-list-item>
      <n-list-item v-for="group in groups.data.items">
        <n-space vertical>
              ...
              <n-gi>
                <n-ellipsis>
                  <n-button
                    ...
                    @click="(e: Event) => clickGroup(e, group)"
                  >
                    <template #icon>
                      <slot name="prefix"></slot>
                    </template>
